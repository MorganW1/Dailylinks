...
test(
  'Scrape from NME and WrestlingInc and save to one markdown with JSON tracking',
  async ({ page }) => {
    // --- Scrape WrestlingInc ---
    ...
    // Load previously scraped URLs or initialize empty array
    let allUrls: string[] = [];
    if (fs.existsSync(jsonPath)) {
      try {
        allUrls = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));
      } catch (e) {
        console.warn('Warning: Could not parse articles.json, starting fresh.');
        allUrls = [];
      }
    }

    let newArticles: { title: string | null; url: string }[] = [];
    ...
    // Save updated URL list for next run
    fs.writeFileSync(jsonPath, JSON.stringify(allUrls, null, 2));
